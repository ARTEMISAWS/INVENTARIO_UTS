...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div>
        ...
        <select name="subcategoria_id" id="subcategoria_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-700 dark:border-gray-600">
            <option value="">Seleccionar</option>
            @foreach($subcategorias ?? [] as $subcategoria)
            <option value="{{ $subcategoria->id }}" data-categoria-id="{{ $subcategoria->categoria_id }}" @selected(old('subcategoria_id', $articulo->subcategoria_id ?? '') == $subcategoria->id)>
                {{ $subcategoria->nombre }}
            </option>
            @endforeach
        </select>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div>
        ...
        <select name="estado" id="estado" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-700 dark:border-gray-600">
            ...
            <option value="inactivo" @selected($articulo->estado == 'inactivo')>Inactivo</option>
        </select>
        @error('estado')
        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
        @enderror
