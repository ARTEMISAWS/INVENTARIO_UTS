<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            {{-- <a href="{{ route('usuarios.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 ...">
                Añadir Usuario
            </a> --}}
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <!-- <form action="{{ route('usuarios.index') }}" method="GET" class="mb-6">
                        <div class="flex items-center space-x-2">
                            <input 
                                ...
                                value="{{ $search ?? '' }}"
                                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200"
                            >
                            <button 
                                type="submit" 
                            ...
                            <tbody>
                                @forelse($usuarios as $usuario)
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">{{ $usuario->name }}</td>
                                        <td class="px-6 py-4">{{ $usuario->cedula }}</td>
                                        <td class="px-6 py-4">{{ $usuario->email }}</td>
                                        {{-- Muestra el primer rol si usas Spatie/Permission, si no, usa $usuario->role --}}
                                        <td class="px-6 py-4">{{ $usuario->role ?? ($usuario->getRoleNames()->first() ?? 'Sin Rol') }}</td>
                                        <td class="px-6 py-4">
                                            {{-- El botón de eliminar es un formulario para enviar el método DELETE --}}
                                            <form action="{{ route('usuarios.destroy', $usuario) }}" method="POST" onsubmit="return confirm('¿Estás seguro de que quieres eliminar este usuario?');">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        <td colspan="5" class="px-6 py-4 text-center">
                                            No hay otros usuarios registrados que coincidan con la búsqueda.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                    ...
                    <div class="mt-4">
                        {{-- La paginación mantiene el parámetro 'search' gracias al controlador --}}
                        {{ $usuarios->links() }}
                    </div>
                </div>
            </div>
